	...
	<div id="content-wrapper">
		<div id="content">
			<div id="content-main">
				...
				<?php if( is_tag() ) { ?><h1 class="cat-heading"><?php _e( 'All posts tagged', 'mvp-text' ); ?> "<?php single_tag_title(); ?>"</h1>
                <?php } else { ?><h1 class="cat-heading">Sessions</h1>
                <?php } ?>
					...
					<div class="widget-home-wrapper">
                        <div id="content-area">
                            ...
                            <?php if (have_posts()) : while(have_posts()) : the_post(); ?>
                                <li><a href="<?php the_permalink() ?>" rel="bookmark"><?php echo ucfirst(get_field('session_type')); ?>: <?php the_title(); ?></a></li>
                            <?php endwhile; endif; ?>
                            </ul>
							...
							<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
								<li class="infinite-post">
									...
									<div class="blog-layout1-text">
										<h2><a href="<?php the_permalink() ?>" rel="bookmark"><?php echo ucfirst(get_field('session_type')); ?>: <?php the_title(); ?></a></h2>
									...
									<div class="blog-layout2-img">
                                        ...
                                        if ($speakers) {
                                            foreach($speakers as $speaker) {
                                                if (has_post_thumbnail($speaker)) {
                                                    ?>
                                                    <a href="<?php get_permalink($speaker->ID) ?>" rel="bookmark"title="<?php get_the_title($speaker->ID); ?>"><?php get_the_post_thumbnail($speaker->ID, 'post-thumb'); ?></a>
                                                    <?php
                                                }
                                            }
                                            ?>
